...
eps = 1e-6
p = 16
obstacle = Expression("-d + pow(pow(x[0]*x[0] + eps, p/2.0) + pow(x[1]*x[1] + eps, p/2.0), 2.0/p)/(2*R)", d=0.0, R=R, eps=eps, p=p, degree=2)
...
file = XDMFFile("contact_2D_animation_cube.xdmf")
